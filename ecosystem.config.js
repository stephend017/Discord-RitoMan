module.exports = {
  ...
  deploy : {
    production : {
      ...
      'post-deploy': "echo 'this is a message purely for testing purposes' && python3.8 -m venv venv && source venv/bin/activate && pip install -r requirements.txt && pip install -e . && alembic revision --autogenerate -m db-update && alembic upgrade head && pm2 reload ecosystem.config.js --env production",
      env: {
        DB_PASS: process.env.DB_PASS,
        RIOT_TOKEN: process.env.RIOT_TOKEN,
        DISCORD_BOT: process.env.DISCORD_BOT,
