...
def _game_end_hook(user: Optional[LoLUser]):
    """
    hook run after game_end rule
    """
...
def _game_start_hook():
    """
    hook run after game_start rule
    """
    return None
...
def run_lol_rules(rule_type: LoLRuleType, user: Optional[LoLUser] = None):
    ...
    if rule_type == LoLRuleType.GAME_START:
        # no builtin behavior here
        _game_start_hook()
